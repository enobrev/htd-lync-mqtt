...
export default class LyncMQTTClient {
    ...
    setHomeAssistantDiscovery(enabled, prefix = 'homeassistant') {
    ...
    updateSourceSelectOptions(zone, sources) {
        if (!this.haDiscoveryEnabled)
            return;
        const zoneDevice = {
    ...
    publishZoneDiscovery(zone, deviceInfo) {
        const zoneDevice = {
            ...deviceInfo,
            identifiers: [`htd_lync_zone_${zone}`],
            name: `HTD Lync Zone ${zone}`,
    ...
    publishMP3Discovery(deviceInfo) {
        const mp3Device = {
            ...deviceInfo,
