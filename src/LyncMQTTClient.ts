...
export default class LyncMQTTClient {
        ...
        try {
            if (topic.startsWith('lync/set/mp3/')) {
                ...
                if (match) {
                    ...
                    const zoneId = zone as Zone;
                    const numValue = parseInt(message, 10);

                    switch(parameter) {
                        case 'name':        await this.Lync.Zone_Name(zoneId, message);                               break;
                        case 'power':       await this.Lync.Zone_Power(zoneId, message === '1');                      break;
                        case 'mute':        await this.Lync.Zone_Mute(zoneId, message === '1');                       break;
                        case 'dnd':         await this.Lync.Zone_DND(zoneId, message === '1');                        break;
                        case 'source':      if (!isNaN(numValue)) await this.Lync.Zone_Source(zoneId, numValue);      break;
                        case 'source_name': await this.handleSourceNameCommand(zoneId, message);                       break;
                        case 'volume':      if (!isNaN(numValue)) await this.Lync.Zone_Volume(zoneId, numValue);      break;
                        case 'treble':      if (!isNaN(numValue)) await this.Lync.Zone_Treble(zoneId, numValue);      break;
                        case 'bass':        if (!isNaN(numValue)) await this.Lync.Zone_Bass(zoneId, numValue);        break;
                        case 'balance':     if (!isNaN(numValue)) await this.Lync.Zone_Balance(zoneId, numValue);     break;
                    }
                }
            }
        } catch (error) {
    ...
    private async handleSourceNameCommand(zone: Zone, sourceName: string) {
